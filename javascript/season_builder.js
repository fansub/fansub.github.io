...
function buildPage(arr, type = TV) {
	...
	if(typeof array === 'undefined' || array.length === 0) {
		...
		for(let i = 0; i < array.length; ++i) {
			...
			dataTable += '<tr>';
			dataTable += '<td><div class="btn-group"><button onclick="copyToClipboard(\'' + encodeText(array[i].name) +'\')" class="btn btn-default btn-outline-secondary" type="button" >' + array[i].name + '</button>';
			dataTable += '<button type="button" onclick="infoKitsu(\'' + encodeText(array[i].name) +'\')" class="btn btn-default btn-outline-secondary dropdown-toggle dropdown-toggle-split" id="btn_' + noSpace(array[i].name) + '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" data-reference="parent"><span class="sr-only">Toggle Dropdown</span></button>';
			dataTable += '<ul class="dropdown-menu" aria-labelledby="btn_' + noSpace(array[i].name) + '>';
